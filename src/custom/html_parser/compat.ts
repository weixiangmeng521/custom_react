...
export function startsWith(str:string, searchString:string, position?:number) {
    return str.slice(position || 0, (position || 0) + searchString.length) === searchString;
  }
  ...
  export function isRealNaN(x:any) {
    return typeof x === 'number' && isNaN(x)
  }
  
  export function arrayIncludes<T>(array:T[], searchElement:T, position?:number) {
